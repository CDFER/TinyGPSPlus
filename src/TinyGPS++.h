...
struct TinyGPSTrackedSatellites {
	uint8_t prn;	   // satellite PRN number listed here http://en.wikipedia.org/wiki/List_of_GPS_satellites
	uint8_t elevation; // in degrees, 0 90 maximum
	uint16_t azimuth;  // degrees from true north, 000 to 359
	uint8_t strength;  // signal strength in dB
};
...
public:
	...
	TinyGPSInteger satellitesTracked;
	TinyGPSTrackedSatellites trackedSatellites[TINYGPS_MAX_SATS];
	TinyGPSHDOP hdop;
